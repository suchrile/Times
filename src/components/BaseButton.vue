<template>
  <button :type="type" :disabled="disabled" class="button" :class="classes">
    <div class="button__content">
      <base-icon
        ...
        class="button__icon"
      />
      <span v-if="text">{{ $filters.lang(text) }}</span>
    </div>
...
export default {
  ...
  computed: {
    classes() {
      ...
      return [this.size, this.view, this.buttonLoading ? 'loading' : '']
    },
    iconSize() {
      if (this.size === 'small') return 16
      else if (this.size === 'medium') return 18
      else if (this.size === 'large' || 'full') return 20
    },
    loaderColor() {
